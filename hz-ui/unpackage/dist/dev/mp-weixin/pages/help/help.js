...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    common_vendor.onShow(() => {
      common_vendor.index.__f__("log", "at pages/help/help.vue:245", "help-create页面显示");
    });
    common_vendor.onHide(() => {
      common_vendor.index.__f__("log", "at pages/help/help.vue:250", "help-create页面隐藏");
      saveDraft();
    });
    const onScroll = (e) => {
    ...
    const uploadImage = async (tempFilePath) => {
      ...
      try {
        ...
        if (result && result.code === 0) {
          ...
          common_vendor.index.__f__("log", "at pages/help/help.vue:305", "上传成功:", formData);
        } else {
          throw new Error(result.msg || "上传失败");
        }
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/help/help.vue:310", "上传图片失败", error);
    ...
    const submitHelp = async () => {
      ...
      try {
        ...
        await utils_request.request.post("/help/help", formData);
        common_vendor.index.showToast({
          title: "发布成功",
          icon: "success",
          duration: 2e3,
        ...
        common_vendor.index.__f__("error", "at pages/help/help.vue:442", "发布失败", error);
        common_vendor.index.showToast({
          title: "发布失败，请重试",
          icon: "error"
        });
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        o: common_vendor.o(chooseImage, "2c")
      } : {}, {
        ...
        r: formData.contact,
        s: common_vendor.o(($event) => formData.contact = $event.detail.value, "a5")
      } : {}, {
        t: formData.contactType === "wechat"
      }, formData.contactType === "wechat" ? {
        v: formData.contact,
        w: common_vendor.o(($event) => formData.contact = $event.detail.value, "1a")
      } : {}, {
        x: formData.contactType === "qq"
      }, formData.contactType === "qq" ? {
        y: formData.contact,
        z: common_vendor.o(($event) => formData.contact = $event.detail.value, "44")
      } : {}, {
        A: scrollTop.value,
        B: common_vendor.o(onScroll, "26"),
        C: !canSubmit.value ? 1 : "",
        D: !canSubmit.value,
        E: common_vendor.o(submitHelp, "03"),
        F: isIphoneX.value ? 1 : "",
        G: loading.value
      }, loading.value ? {
        H: common_vendor.p({
