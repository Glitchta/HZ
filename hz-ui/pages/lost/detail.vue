    ...
    <scroll-view class="detail-content" scroll-y>
      ...
      <view class="detail-card">
		...
		<view class="user-info">
		  <image 
		    class="user-avatar" 
		    :src="lostDetail.avatar || '/static/default-avatar.png'" 
		    mode="aspectFill"
		  ></image>
		  <view class="user-detail">
		    <text class="user-name">{{ lostDetail.nickname }}</text>
			<text class="help-time">{{formatTime(lostDetail.createTime)}}</text>
		  </view>
		</view>
		  
        <!-- 内容 -->
        <view class="detail-text">
          <text>{{lostDetail.description}}</text>
        </view>
        
        <!-- 图片展示 -->
        <view class="detail-images" v-if="lostDetail.images && lostDetail.images.length > 0">
          <view 
            v-for="(img, index) in lostDetail.images" 
            :key="index"
        ...
        <!-- 标签 -->
        <view class="detail-tags" v-if="lostDetail.tags && lostDetail.tags.length > 0">
          <view 
            v-for="tag in lostDetail.tags" 
            :key="tag"
            class="detail-tag"
          >
            {{ tag }}
        ...
        <view class="detail-meta">
          <view class="meta-stats">
            <view class="meta-stat" @click="toggleLike">
              <uni-icons
                :type="lostDetail.isLiked ? 'heart-filled' : 'heart'"
                :color="lostDetail.isLiked ? '#ff6b6b' : '#999'"
                size="16"
              ></uni-icons>
              <text>{{ lostDetail.likeCount || 0 }}</text>
            </view>
...
const checkCollectionStatus = async () => {
  try {
    const res = await request.post('/collection/check', { contentId: String(lostId.value), contentType: 'lost' })
...
const checkLikeStatus = async () => {
  try {
    const res = await request.post('/like/check', { contentId: String(lostId.value), contentType: 'lost' })
...
const handleCollect = async () => {
  ...
  try {
    if (isCollected.value) {
      await request.post('/collection/cancel', { contentId: lostId.value, contentType: 'lost' })
      isCollected.value = false
      uni.showToast({ title: '已取消收藏' })
    } else {
      await request.post('/collection/add', { contentId: lostId.value, contentType: 'lost' })
...
const fetchlostDetail = async () => {
  try {
    const res = await request.get('/lost/getById', {
...
const showReplyInput = (comment, type, reply = null) => {
  ...
  if (type === 'comment') {
    // 回复一级评论
    replyTargetName.value = comment.nickname || '匿名用户'
...
const submitReply = async () => {
  ...
  try {
	...
	//修改评论数
	await request.put('/lost/comment',null,{params:{id : lostId.value}})
...
const deleteComment = (comment) => {
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        try {
          const result = await request.post('/comment/delete', { commentId: comment.id, contentType: 'lost' })
          if (result.code === 0) {
...
const deleteReply = (parentComment, reply) => {
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        try {
          const result = await request.post('/comment/delete', { commentId: reply.id, contentType: 'lost' })
          if (result.code === 0) {
...
const toggleLike = async () => {
  ...
  try {
    ...
    if (res.code === 0) {
      ...
      lostDetail.value.likeCount = liked
        ? (lostDetail.value.likeCount || 0) + 1
        : Math.max(0, (lostDetail.value.likeCount || 1) - 1)
    }
  } catch (error) {
