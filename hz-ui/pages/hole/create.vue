    ...
    <scroll-view class="publish-content" scroll-y>
      ...
      <view class="form-container">
        ...
        <view class="word-count">
          <text>{{ formData.content.length || 0 }}/1000</text> <!-- 添加 || 0 避免初始值为空时的错误 -->
        </view>
        ...
        <view class="notice">
          ...
          <view class="notice-list">
            <text>1. 树洞内容完全匿名，保护您的隐私</text>
            <text>2. 请遵守社区规范，禁止发布违法、侵权内容</text>
            <text>3. 支持上传最多9张图片</text>
          </view>
        </view>
      </view>
    </scroll-view>
...
// 删除图片
const removeImage = (index) => {  // 方法名已统一为 removeImage
  uni.showModal({
    title: '提示',
    content: '确定要删除这张图片吗？',
    success: async (res) => {
...
const publishHole = async () => {
  ...
  try {
    // 提交树洞
    const res = await request.post('/hole/insert', formData)
    ...
    if (res.code === 0) {  // 根据你的后端接口返回码调整
      ...
      // 设置需要刷新列表
      uni.setStorageSync('refreshHoleList', true)
